...
- Auto complete suggestions from any JSON API (async with debouncing)
...
# Async suggestions configuration (default: 150ms)
...
Suggestions are fetched asynchronously with configurable debouncing to improve performance:

- **suggestions_debounce**: Delay (in milliseconds) before fetching suggestions after typing stops. Default: 150ms

Previous suggestions are shown as placeholders and updated with actual suggestions after the debounce delay. If you type quickly, only the latest query's suggestions will be fetched.
